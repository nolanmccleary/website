...
Wake up, we're at execution cycles! An execution cycle is simply an atomic state transition period inside the system. As such, the clock speed is simply the number of execution cycles that occur per unit time and thus the rate at which the system changes state. A program's runtime complexity is simply how many state changes it takes to get the system to go from its start state to its end state as a function of input whereas its memory complexity is how much of the system's memory it disrupts while doing this, also as a function of input. The output of the program is simply the state of the subset of memory that we care about when the end state is reached.
